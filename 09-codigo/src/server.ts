  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/seeds/select') {
        const input = await body(req);
        const seeds = (await store.read()).seeds;
      ...
      if (req.method === 'POST' && url.pathname === '/api/farmer/profile') {
        ...
        const profile = createFarmerProfile(seed, input.selectedThemes || []);
        await store.append('farmer_profiles', profile);
        return json(res, 201, profile);
      }
      if (req.method === 'GET' && url.pathname === '/api/farmer/profiles') {
        return json(res, 200, (await store.read()).farmer_profiles || []);
      }
      if (req.method === 'POST' && url.pathname === '/api/farmer/profile/approve') {
        const input = await body(req);
        const profile = await find(store, 'farmer_profiles') as any[];
        const current = profile.find((p: any) => p.id === input.profileId);
      ...
      if (req.method === 'POST' && url.pathname === '/api/post-machine/generate') {
        ...
        const topics = input.topics || {
          blog: profile.socialContentIdeas.blog.slice(0, 2),
          video: profile.socialContentIdeas.video.slice(0, 1),
          shorts: profile.socialContentIdeas.shorts.slice(0, 2),
          stories: profile.socialContentIdeas.stories.slice(0, 2),
        };
        const output = await generatePostMachineOutput(profile, input.week || 1, topics);
        await store.append('post_machine', output);
        return json(res, 201, output);
      }
      if (req.method === 'GET' && url.pathname === '/api/post-machine') {
        return json(res, 200, (await store.read()).post_machine || []);
      }
